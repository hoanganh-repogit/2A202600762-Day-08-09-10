...
def supervisor_node(state: AgentState) -> AgentState:
    ...
    Routing cố ý rule-based để trace dễ đọc và chạy ổn định trong lab.
    """
    task = state["task"].lower()
    state["history"].append(f"[supervisor] received task: {state['task'][:80]}")

    route = "retrieval_worker"
    route_reason = "default retrieval: factual lookup in internal docs"
    needs_tool = False
    risk_high = False

    access_keywords = ["cấp quyền", "access", "level 2", "level 3", "level 4", "admin access", "contractor"]
    ...
    simple_refund_fact = "hoàn tiền" in task and any(kw in task for kw in ["bao nhiêu ngày", "bao nhieu ngay"])
    risk_keywords = ["emergency", "khẩn cấp", "2am", "không rõ", "err-"]

    if any(kw in task for kw in access_keywords):
        route = "policy_tool_worker"
        route_reason = "task contains access-control keyword; policy worker must call MCP tools"
        needs_tool = True
    elif any(kw in task for kw in policy_decision_keywords):
        route = "policy_tool_worker"
        route_reason = "task asks for policy decision/exception; policy worker must call MCP tools"
        needs_tool = True
    elif simple_refund_fact:
        route = "retrieval_worker"
        route_reason = "simple refund fact question; retrieval evidence is enough"
    elif any(kw in task for kw in ["p1", "sla", "ticket", "escalation", "mật khẩu", "remote", "probation"]):
        route = "retrieval_worker"
        route_reason = "task contains factual SLA/IT/HR keyword; retrieval worker selected"
...
def human_review_node(state: AgentState) -> AgentState:
    ...
    Optional future improvement: implement actual HITL với interrupt_before
    hoặc breakpoint nếu dùng LangGraph.
    """
...
def build_graph():
    ...
    Lab này implement Option A theo mặc định; có thể chuyển sang LangGraph nếu muốn.
    """
    ...
    def run(state: AgentState) -> AgentState:
        ...
        if route == "human_review":
            ...
            # Retrieval vẫn chạy sau policy để synthesis có evidence từ worker riêng,
            # còn policy worker chứng minh MCP path bằng mcp_tools_used.
            state = retrieval_worker_node(state)
        else:
